    ...
    useEffect(() => {
        async function fetchRepos() {
            try {
                ...
                const formattedRepos = data.map((repo: { name: string; description: string; id: number }) => ({
                    name: repo.name,
    ...
    return (
        <main className="flex flex-col items-center justify-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {repos.map((repo) => (
                    <div key={repo.id} className="border p-4 rounded border-black dark:border-white">
